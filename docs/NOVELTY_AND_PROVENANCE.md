...
> **Status note (September 2026, added after the cross-threshold extension).** This document records
> the audit as it stood before commit `910ce1c`. Three of its statements have since been superseded
> by the Lean development. They are left in place below, unedited, as a record of what the audit
> found at the time:
>
> 1. §3 and §8 state that the chain "OU model ⟹ `A = det Σ`" is unformalized. Its *algebraic* half
>    is now machine-checked: `OUCorridor/Covariance.lean` proves that the explicit `Σ` of paper
>    eqs. (36)–(38) is the unique solution of `M Σ + Σ Mᵀ = -Q` for `κ ≥ 0` (`cov_lyapunov`,
>    `lyapunov_unique`) and that `det Σ = volume` (`det_cov`). The stochastic half — that the
>    process's stationary covariance satisfies that equation (paper Proposition 1) — remains
>    unformalized, so the audit's central caveat still stands.
> 2. §11 records `OUCorridor/Basic.lean` as **removed**. It was not removed. The file is retained,
>    and is recorded as one of the six modules in `verified/cross-threshold-2026-09/`.
> 3. §12, item 3 states that the residual threshold `r_D* = 2 + √3` is unformalized and was not
>    audited. Its criterion is now formalized as `exists_residual_improvement_iff_threshold`
>    (paper Proposition 5).
>
> The audit's novelty verdict, literature findings, and interpretive conclusions are unaffected.
